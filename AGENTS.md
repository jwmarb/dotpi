...
- `agent/fff/`, `agent/npm/`, `agent/git/`, `agent/pi-blackhole/`, `agent/sessions/` — machine state, all gitignored
...
| Loop the agent on a goal until it declares completion | `agent/extensions/ralph-loop/` (`/ralph-loop`, `/ulw-loop`, `/loop-stop`; `--verify[=static\|runtime\|both]` gates completion on an oracle audit and/or a containerised test + UI run) |
| The reference container image the runtime gate verifies inside | `agent/docker/verify-base.Dockerfile` (tracked; browser + agent-browser CLI, tag `ralph-verify/base:latest`) |
...
- **Parse errors are startup-fatal.** Pi auto-loads every top-level `agent/extensions/*.ts`; a test file placed there (it imports `bun:test`) breaks pi startup. Tests live in subdirectories — currently `agent/extensions/lib/{widget,agents,layout,sessions}.test.ts` and `agent/extensions/subagent-herdr/lib.test.ts`.
...
Some of the test files import packages pi normally injects itself (`@earendil-works/pi-tui` in the widget tests; `typebox` and pi-coding-agent types via `child-done.ts` in the herdr tests), so `bun test` — which has no access to pi's alias map — only resolves them via a gitignored root `node_modules/` of symlinks into bun's global tree:
...
- `agent/docker/verify-base.Dockerfile` is the **tracked** reference image for runtime verification (Node 24 + Chrome + `agent-browser`); generated project images extend it via `FROM ralph-verify/base:latest`. `agent/verify-images/` holds the generated per-project Dockerfiles (gitignored; rebuildable). Runtime verification needs a working Docker daemon — without one the gate returns `inconclusive` and the loop stops rather than assuming success.
...
- Nested AGENTS.md: `agent/extensions/subagent-herdr/AGENTS.md` (module internals — herdr CLI protocol, completion handshake, run-directory contract); `agent/extensions/ralph-loop/AGENTS.md` (the `agent_settled` loop contract, why continuations must defer a macrotask, and the `--verify` gates' fail-stop policy + container contract); `agent/git/github.com/k0valik/pi-blackhole/AGENTS.md` — the vendored package's own knowledge base (pnpm test/typecheck/lint/build), applies inside that checkout only.
