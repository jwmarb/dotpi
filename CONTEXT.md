...
**Injected prompt**:
The content the orchestrator assembles and hands to a **Run**: the subagent's own system prompt, any preloaded skills, and — on the **Fallback path** only — the conditional result contract; a **Native Run** never receives it, because its **Result** is read from the **Transcript** rather than scraped from a fenced tag. It is exactly what crosses `--append-system-prompt`, left unarchived like **Run Meta**. For a delegated `sub-` **Run** the spawn record `prompt.md` is the **Injected prompt** itself; for a plan-spawned `pln-` **Run** (autonomous review, Rework) the **Injected prompt** is the agent definition alone, because the **Delegation brief** crosses as the child's first user message rather than through this channel — so such a **Run**'s `prompt.md` records the whole delegation payload, the **Injected prompt** plus the brief marked as the first user message (ADR 0047). Distinct from the child's *final* system prompt, which also includes pi's own base prompt: capturing that would require patching pi itself, and it would go stale on every pi release.
_Avoid_: system prompt (overloaded — the child's final prompt includes pi's base), append prompt

**Delegation brief**:
The per-**Run** work text a child is asked to do — the `task` parameter of a delegated **Run**, or the rework/review contract of a plan-spawned one. It crosses to the child as its first user message on every **Run** and on both spawn paths, so the **Transcript** records what the **Run** was asked, visible in a **Run Pane** and a **Mirror Pane** alike. The crossing is verbatim — no prefix — for plan-spawned **Runs**, which ADR 0047 decided, and for native delegated **Runs**, whose task is the child's positional prompt; a delegated **Fallback path** **Run** prefixes it with `Task: `, so the first user message recorded for such a **Run** is `Task: <brief>`, not the brief alone. Distinct from the **Injected prompt**, the system-prompt channel: the agent definition, preloaded skills, and the result contract.
_Avoid_: task prompt, the prompt (overloaded)
...
**Starter Plan**:
A **Plan** the orchestrator authors for a subagent before delegating work. Its items cross into the subagent's plan file at spawn — never inside the **Delegation brief** — so the child's plan tool and the parent's **Board** address the same file (ADR 0048). A fixed nudge line also crosses inside the **Delegation brief**, telling the child the plan exists and is part of the job; the subagent owns and maintains it from that moment on, and the orchestrator never reads it back.
...
The published on-disk layout `<agentDir>/subagent-sessions/<runId>/` — a **Transcript**, its **Run Meta**, and `prompt.md` recorded at spawn: the **Injected prompt** for every **Run**, and for a plan-spawned `pln-` **Run** additionally the **Delegation brief**, marked as the first user message, so the directory answers what the **Run** was asked on its own — shared by both producers of **Runs** and read by three consumers that hold no handle on the producing process: the **Run Index**, **Board** progress and the **Mirror Pane** (ADR 0045, ADR 0047). The subagent extension mints `sub-` **Run** IDs, the plan extension mints `pln-` ones for its autonomous reviews, and both write the twice-written **Run Meta** sidecar through one module so it cannot be forgotten by one of them (ADR 0039). A `pln-` prefix in `/runs` is how to tell a plan-spawned **Run** from a delegated one.
